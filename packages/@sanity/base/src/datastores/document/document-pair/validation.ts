...
const cacheOn = createObservableCache<ValidationStatus>()

export function validationFor(publishedId: string, typeName: string) {
  return concat(
    of(INITIAL_VALIDATION_STATUS),
    documentStore.local.editStateOf(publishedId, typeName).pipe(
      debounceTime(300),
      switchMap(editState =>
        concat(
